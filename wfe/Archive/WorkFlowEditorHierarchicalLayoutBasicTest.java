...
public class WorkFlowEditor extends JFrame implements ActionListener 
{
   ...
   private JMenuItem aboutMenuItem; 
        ...
        // add extra vertex that is connected 
        // to all unconnected vertices in order 
        // to improve layout of the graph 
   mxCell hiddenVertex; 
   List<mxCell> hiddenEdges ; 
	...
	public WorkFlowEditor()
	{
      ...
      setJMenuBar(menuBar);
		...
		{
         hiddenVertex = (mxCell) 
            graph.insertVertex(parent, null, new Activity(" "), 20, 20, 80, 30 );
         hiddenVertex.setVisible(false) ;
         hiddenEdges = new ArrayList<mxCell> () ; 
   ...
   private void doFastOrganicGraphLayout() { 
      for( int i = 0; i < 100; i++ ) { 
   ...
   private void doHierarchicalLayout() { 
   ...
   private void addVertex(int x, int y) { 
      ...
      try { 
         graph.getModel().beginUpdate();      
         mxCell addedVertex = (mxCell) graph.insertVertex( parent, 
                                                           null, 
                                                           new Activity(cellname), 
                                                           x, y, 80, 30 );
         graph.insertEdge(hiddenVertex, null, " ", hiddenVertex, addedVertex);
              // at this stage there is no edge to this vertex 
      }
      finally {
         graph.getModel().endUpdate();
      }
